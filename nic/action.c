...
static
zt_s32 action_ba_req_work_ap(nic_info_st *pnic_info, wdn_net_info_st *pwdn_info)
{
    if (pwdn_info == NULL)
    {
                               ...
                               zt_80211_mgmt_t *pmgmt, zt_u16 mgmt_len)
{
    ...
    {
        if (mlme_info->connect == zt_false &&  nic_info->p2p == NULL)
        {
            return 0;
        }
    }
                                      ...
                                      struct xmit_frame *pxmitframe)
{
    ...
    if ((pwdn_info->htpriv.mcu_ht.ht_option == zt_true) &&
            (pwdn_info->htpriv.mcu_ht.ampdu_enable == zt_true))
    {
        issued = (pwdn_info->htpriv.mcu_ht.agg_enable_bitmap >> pxmitframe->priority) &
                 0x1;
        issued |= (pwdn_info->htpriv.mcu_ht.candidate_tid_bitmap >>
                   pxmitframe->priority) &
                  0x1;
        if (issued == 0)
        {
            if ((pxmitframe->frame_tag == DATA_FRAMETAG) &&
                    (pxmitframe->ether_type != 0x0806) &&
...
zt_s32 zt_action_frame_del_ba_request(nic_info_st *nic_info)
{
    wdn_net_info_st *wdn_net_info   = NULL;
    mlme_info_t *mlme_info          = (mlme_info_t *)nic_info->mlme_info;

    wdn_net_info  = zt_wdn_find_info(nic_info, zt_wlan_get_cur_bssid(nic_info));

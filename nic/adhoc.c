                                 ...
                                 sys_work_mode_e network_type)
{
    ...
    {
        zt_io_write_cmd_by_mailbox(pnic_info, UMSG_OPS_HAL_SET_BCN, val, 2, NULL, 0);
        zt_io_write_cmd_by_mailbox(pnic_info, UMSG_OPS_HW_SET_MLME_DISCONNECT, tmp, 2,
                                   NULL, 0);
                             ...
                             wdn_net_info_st *pwdn_info)
{
    ...
    zt_adhoc_connect_media_status(pnic_info, pwdn_info);
                          ...
                          zt_s32 reason)
{
    ...
    if (reason == 0)
    {
        ...
        ADHOC_ARRAY(pcur_network->mac_addr, ZT_80211_MAC_ADDR_LEN);
        if (pwdn_info == NULL)
        {
        ...
        zt_wdn_data_update(pnic_info, pwdn_info);
    ...
    /* enable data queue */
    zt_mcu_set_correct_tsf(pnic_info, 0);
...
zt_s32 zt_adhoc_keepalive_thrd(nic_info_st *pnic_info)
{
    ...
    zt_list_for_each_safe(pos, pos_next, &pwdn->head)
    {
        ...
        if (!zt_80211_is_bcast_addr(pwdn_info->mac))
        {
            ...
            adhoc_info->asoc_sta_count--;
            zt_wdn_remove(pnic_info, pwdn_info->mac);
            zt_adhoc_disconnect_media_status(pnic_info, pwdn_info);

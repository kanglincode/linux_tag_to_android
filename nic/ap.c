...
#define ZT_AP_MSG_BA_RSP_SIZE_MAX   ZT_OFFSETOF(zt_ap_msg_t, mgmt)
...
#define RX_IDLE_TIMEOUT_SECONDS     (20 * 60) /* 20 minutes */
            ...
            {
                AP_WARN("STA timeout idle->"ZT_MAC_FMT, ZT_MAC_ARG(pwdn_info->mac));
        ...
        else if (pmsg->tag == ZT_AP_MSG_TAG_DISASSOC_FRAME)
        {
            ...
            if (pmsg && pmsg->tag == ZT_AP_MSG_TAG_ASSOC_REQ_FRAME)
            {
                ...
                zt_ap_disconnect_media_status(pnic_info, pwdn_info);
            }
            break;
        }
        ...
        else if (pmsg->tag == ZT_AP_MSG_TAG_BA_REQ_FRAME)
        {
            ...
            zt_action_frame_ba_to_issue(pnic_info,
                                        ZT_WLAN_ACTION_ADDBA_RESP);
        }
...
static zt_inline zt_s32 new_boradcast_wdn(nic_info_st *pnic_info)
{
    ...
    zt_que_init(&pwdn_info->ap_msg, ZT_LOCK_TYPE_SPIN);
    pwdn_info->aid = 0;
    ...
    zt_ap_connect_media_status(pnic_info, pwdn_info);
...
static void ap_poll(nic_info_st *pnic_info)
{
    ...
    for (;;)
    {
        ...
        do
        {
            /* poll ap thread by each wdn_info */
            zt_list_t *pos, *pos_next;
            wdn_list *pwdn = pnic_info->wdn;
            zt_list_for_each_safe(pos, pos_next, &pwdn->head)
            {
            ...
            zt_msleep(1);
        } while (zt_ap_status_get(pnic_info) == ZT_AP_STATE_ESTABLISHED);
    }
...
static void ap_update_reg(nic_info_st *pnic_info)
{
    ...
    if (pcur_network->ht_enable == zt_true)
    {
        ...
        zt_mcu_set_min_ampdu_space(pnic_info,
                                   (pcur_network->pht_cap.ampdu_params_info &
                                    ZT_80211_MGMT_HT_AMPDU_PARM_DENSITY) >>
                                   ZT_80211_MGMT_HT_AMPDU_PARM_DENSITY_SHIFT);
    }
...
#define RX_BROADCAST_USE_DEF_KEY    ZT_BIT(7)
    ...
    zt_hw_info_set_channnel_bw(pnic_info,
    ...
    if (pht_cap)
    {
                 ...
                 (pht_cap->cap_info & ZT_80211_MGMT_HT_CAP_RX_STBC))
        {
            ...
            pht_cap->cap_info |= (phw_info->rx_stbc_num <<
                                  ZT_80211_MGMT_HT_CAP_RX_STBC_SHIFT) &
...
void zt_ap_update_beacon(nic_info_st *pnic_info, zt_u8 ie_id, zt_u8 *oui,
                         zt_u8 tx)
{
    if (!pnic_info)
    {

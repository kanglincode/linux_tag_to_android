...
zt_s32 zt_wlan_mgmt_scan_que_read_try(zt_wlan_mgmt_scan_que_t *pscan_que)
{
    ...
    zt_lock_lock(&pscan_que->lock);
    if (pscan_que->read_cnt == 0xFF)
    {
        zt_lock_unlock(&pscan_que->lock);
        return -2;
    }
                                ...
                                zt_wlan_mgmt_scan_que_node_t *pscan_que_node)
{
    ...
    zt_80211_mgmt_ht_operation_t *pht_opt;
    ...
    pscan_que_node->privacy =
        (zt_bool)(!!(pmgmt->beacon.capab & ZT_80211_MGMT_CAPAB_PRIVACY));
        ...
        switch (pie->element_id)
        {
            case ZT_80211_MGMT_EID_SSID :
                ...
                else
                {
                    ...
                    pscan_que_node->ssid_type =
                        pie->len == 0 ? ZT_80211_HIDDEN_SSID_ZERO_LEN :
                        pie->data[0] == '\0' ? ZT_80211_HIDDEN_SSID_ZERO_CONTENTS :
                        ZT_80211_HIDDEN_SSID_NOT_IN_USE;

                    /* update ssid information */
                    if (pscan_que_node->ssid_type == ZT_80211_HIDDEN_SSID_NOT_IN_USE)
                    {
                        pscan_que_node->ssid.length = pie->len;
                        zt_memcpy(pscan_que_node->ssid.data, pie->data, pie->len);
                        pscan_que_node->ssid.data[pie->len] = '\0';
            ...
            case ZT_80211_MGMT_EID_VENDOR_SPECIFIC :
                if (!zt_memcmp(pie->data, wpa_oui, sizeof(wpa_oui)))
                {
                    ...
                    {
                        zt_memcpy(&pscan_que_node->wpa_ie, pie,
            ...
            case ZT_80211_MGMT_EID_RSN :
                ...
                {
                    zt_memcpy(&pscan_que_node->rsn_ie, pie,
                              ZT_OFFSETOF(zt_80211_mgmt_ie_t, data) + pie->len);
                }
                else
                {
    ...
    {
        pscan_que_node->name =
            pscan_que_node->ht_spot ? ZT_WLAN_BSS_NAME_IEEE80211_BGN :
            ZT_WLAN_BSS_NAME_IEEE80211_BG;
    }
    else if (cck_spot)
    {
        pscan_que_node->name =
            pscan_que_node->ht_spot ? ZT_WLAN_BSS_NAME_IEEE80211_BN :
            ZT_WLAN_BSS_NAME_IEEE80211_B;
    }
    else
    {
        pscan_que_node->name =
            pscan_que_node->ht_spot ? ZT_WLAN_BSS_NAME_IEEE80211_GN :
    ...
    while (zt_os_api_thread_wait_stop(pwlan_mgmt_info->tid) == zt_false)
    {
        ...
        switch (pmsg->tag)
        {
            ...
            case ZT_WLAN_MGMT_TAG_ACTION :
            {
                ...
                /* retrive wdn_info */
                pwdn_info = zt_wdn_find_info(pnic_info, pfrm_msg->mgmt_frm.sa);
                zt_action_frame_process(pnic_info, pwdn_info, &pfrm_msg->mgmt_frm, mgmt_len);
                break;
            }
                           ...
                           zt_80211_mgmt_t *pmgmt, zt_u32 mgmt_len)
{
    ...
    /* fill message value */
    {
        ...
        zt_memcpy(&prx_frm_msg->mgmt_frm, pmgmt, mgmt_len);
    }
...
zt_s32 zt_wlan_mgmt_rx_frame(void *ptr)
{
    ...
    switch (frm_type)
    {
        ...
        case ZT_80211_FRM_PROBE_REQ :
            ...
            if (get_sys_work_mode(pnic_info) == ZT_MASTER_MODE)
            {
...
static zt_s32 wlan_mgmt_scan_que_init(nic_info_st *pnic_info)
{
    ...
    zt_lock_init(&pscan_que->lock, ZT_LOCK_TYPE_IRQ);

...
static zt_s32 p2p_scan_ie_parse(nic_info_st *pnic_info, zt_s8 *buf, zt_u32 len)
{
    ...
    if (len > 0)
    {
        if ((wps_ie = zt_wlan_get_wps_ie((zt_u8 *)buf, len, NULL, &wps_ielen)))
        {
            ...
            {
                zt_kfree(pmlme_info->probereq_wps_ie);
                pmlme_info->probereq_wps_ie = NULL;
                pmlme_info->wps_ie_len = 0;
            }

            pmlme_info->probereq_wps_ie = zt_kzalloc(wps_ielen);
            if (pmlme_info->probereq_wps_ie == NULL)
            {
                P2P_DBG("zt_kzmalloc probereq_wps_ie failed");
                return -1;
            }
            zt_memcpy(pmlme_info->probereq_wps_ie, wps_ie, wps_ielen);
...
static zt_s32 p2p_beacon_parse(nic_info_st *pnic_info, zt_s8 *buf, zt_s32 len)
{
    ...
    if ((wps_ie = zt_wlan_get_wps_ie((zt_u8 *)buf, len, NULL, &wps_ielen)))
    {
        ...
        {
            zt_kfree(pmlme_info->wps_beacon_ie);
            pmlme_info->wps_beacon_ie = NULL;
            pmlme_info->wps_beacon_ie_len = 0;
        }
        pmlme_info->wps_beacon_ie = zt_kzalloc(wps_ielen);
        if (pmlme_info->wps_beacon_ie == NULL)
        {
            P2P_ERR("wps_beacon_ie zt_kzalloc failed! ");
            return -1;
        }
        zt_memcpy(pmlme_info->wps_beacon_ie, wps_ie, wps_ielen);
    ...
    if (len > 0)
    {
        if ((wps_ie = zt_wlan_get_wps_ie((zt_u8 *)buf, len, NULL, &wps_ielen)))
        {
            ...
            if (pmlme_info->wps_probe_resp_ie)
            {
                ...
                zt_kfree(pmlme_info->wps_probe_resp_ie);
                pmlme_info->wps_probe_resp_ie = NULL;
            }

            pmlme_info->wps_probe_resp_ie = zt_kzalloc(wps_ielen);
            ...
            zt_memcpy(pmlme_info->wps_probe_resp_ie, wps_ie, wps_ielen);
                                  ...
                                  zt_s32 len)
{
    ...
    if ((wps_ie = zt_wlan_get_wps_ie((zt_u8 *)buf, len, NULL, &wps_ielen)))
    {
        ...
        {
            zt_kfree(pmlme_info->wps_assoc_resp_ie);
            pmlme_info->wps_assoc_resp_ie = NULL;
            pmlme_info->wps_assoc_resp_ie_len = 0;
        }
        pmlme_info->wps_assoc_resp_ie = zt_kzalloc(wps_ielen);
        if (pmlme_info->wps_assoc_resp_ie == NULL)
        {
            P2P_ERR("zt_kzalloc wps_assoc_resp_ie failed!");
            return -1;
        }
        zt_memcpy(pmlme_info->wps_assoc_resp_ie, wps_ie, wps_ielen);

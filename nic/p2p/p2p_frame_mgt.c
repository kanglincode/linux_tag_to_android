                         ...
                         zt_u8 flag)
{
    ...
    /*4. wps ie and p2p ie*/
    {
        ...
        if (mlme_info && mlme_info->probereq_wps_ie && mlme_info->wps_ie_len)
        {
            ...
            zt_memcpy(pvar, mlme_info->probereq_wps_ie, mlme_info->wps_ie_len);
...
zt_s32 p2p_send_probersp(nic_info_st *pnic_info, unsigned char *da, zt_u8 flag)
{
    ...
    if (pmlme_info->wps_probe_resp_ie != NULL &&
            p2p_info->p2p_ie[ZT_P2P_IE_PROBE_RSP] != NULL)
    {
        zt_memcpy(pvar, pmlme_info->wps_probe_resp_ie,
                          ...
                          p2p_frame_check_param_st *check_param)
{
    ...
    p2p_dump_attr_ch_list(check_param->p2p_ie, check_param->p2p_ielen, ch_list_buf,
                          128, 1);
    zt_hw_info_get_channnel_bw(pnic_info, &ch, &bw, &offset);
                                ...
                                zt_u32 len)
{
    ...
    zt_hw_info_get_channnel_bw(pnic_info, &hw_ch, &cw, &offset);
    ...
    if (P2P_GO_NEGO_CONF == type || P2P_PROVISION_DISC_RESP == type ||
            P2P_INVIT_RESP == type)
    {
                         ...
                         zt_u8 wait_ack)
{
    ...
    if (wait_ack)
    {
        ret = zt_nic_mgmt_frame_xmit_with_ack(pnic_info, NULL, pxmit_buf, len);
    }
    else
    {
                                ...
                                zt_u8 ch, zt_u8 wait_ack)
{
    ...
    zt_hw_info_get_channnel_bw(pnic_info, &hw_ch, &bw, &offset);
    if (ch != hw_ch)
    {
        //        zt_wlan_set_cur_channel(pnic_info, ch);
        zt_hw_info_set_channnel_bw(pnic_info, ch, CHANNEL_WIDTH_20,

...
#define RSNE_GROUP_DATA_ZT_CIPHER_SUITE_SIZE       4
#define RSNE_PARIWISE_ZT_CIPHER_SUITE_COUNT_SIZE   2
#define RSNE_PARIWISE_ZT_CIPHER_SUITE_LIST_SIZE    4
    ...
    if (left < RSNE_GROUP_DATA_ZT_CIPHER_SUITE_SIZE)
    {
    ...
    left -= RSNE_GROUP_DATA_ZT_CIPHER_SUITE_SIZE;
    pos  += RSNE_GROUP_DATA_ZT_CIPHER_SUITE_SIZE;
    if (left == 0)
    {
    ...
    if (left < RSNE_PARIWISE_ZT_CIPHER_SUITE_COUNT_SIZE)
    {
    ...
    left -= RSNE_PARIWISE_ZT_CIPHER_SUITE_COUNT_SIZE;
    pos  += RSNE_PARIWISE_ZT_CIPHER_SUITE_COUNT_SIZE;
    /* get pairwise cipher suite */
    if (cipher_suite_cnt == 0 ||
            left < cipher_suite_cnt * RSNE_PARIWISE_ZT_CIPHER_SUITE_LIST_SIZE)
    {
        ...
        *pairwise_cipher = 0x0;
        do
        {
            *pairwise_cipher |= rsne_cipher_suite_parse(zt_be32_to_cpu(*(zt_u32 *)pos));
            pos += RSNE_PARIWISE_ZT_CIPHER_SUITE_LIST_SIZE;
                                ...
                                zt_u32 *pgroup_cipher, zt_u32 *pairwise_cipher)
{
    ...
    do
    {
        ret = zt_80211_mgmt_rsn_parse(pie, ie_len,
...
#define WPA_MULTICAST_ZT_CIPHER_SUITE_SIZE         4
#define WPA_UNICAST_ZT_CIPHER_SUITE_COUNT_SIZE     2
#define WPA_UNICAST_ZT_CIPHER_SUITE_LIST_SIZE      4
    ...
    if (left < WPA_MULTICAST_ZT_CIPHER_SUITE_SIZE)
    {
    ...
    left -= WPA_MULTICAST_ZT_CIPHER_SUITE_SIZE;
    pos += WPA_MULTICAST_ZT_CIPHER_SUITE_SIZE;
    if (left == 0)
    {
    ...
    if (left < WPA_UNICAST_ZT_CIPHER_SUITE_COUNT_SIZE)
    {
    ...
    left -= WPA_UNICAST_ZT_CIPHER_SUITE_COUNT_SIZE;
    pos += WPA_UNICAST_ZT_CIPHER_SUITE_COUNT_SIZE;
    /* get unicast cipher suite */
    if (cipher_suite_cnt == 0 ||
            left < cipher_suite_cnt * WPA_UNICAST_ZT_CIPHER_SUITE_LIST_SIZE)
    {
        _80211_WARN("cipher suite count(%d) error", cipher_suite_cnt);
        ret = -7;
        goto exit;
        ...
        *punicast_cipher = 0x0;
        do
        {
            *punicast_cipher |= get_wpa_cipher_suite(zt_be32_to_cpu(*(zt_u32 *)pos));
            pos += WPA_UNICAST_ZT_CIPHER_SUITE_LIST_SIZE;
        } while (--cipher_suite_cnt);
    }
                                ...
                                zt_u32 *pmulticast_cipher, zt_u32 *punicast_cipher)
{
    ...
    do
    {
        ret = zt_80211_mgmt_wpa_parse(pie, ie_len,

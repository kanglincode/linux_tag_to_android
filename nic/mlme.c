...
zt_s32 hw_cfg(nic_info_st *pnic_info, wdn_net_info_st *wdn_info)
{
    ...
    ret |= zt_mcu_set_correct_tsf(pnic_info, wdn_info->tsf);

    ret |= zt_mcu_set_sifs(pnic_info);

    ret |= zt_mcu_set_macid_wakeup(pnic_info, wdn_info->wdn_id);

    ret |= zt_mcu_set_max_ampdu_len(pnic_info,
                                    wdn_info->htpriv.mcu_ht.rx_ampdu_maxlen);
    ret |= zt_mcu_set_min_ampdu_space(pnic_info,
    ...
    ret |= zt_hw_info_set_channnel_bw(pnic_info,
                                      wdn_info->channel,
                                      wdn_info->bw_mode,
...
zt_inline static zt_s32 build_wdn(nic_info_st *pnic_info)
{
    ...
    if (zt_wdn_data_update(pnic_info, pwdn_info))
    {
...
core_conn_preconnect(zt_pt_t *pt, nic_info_st *pnic_info, zt_s32 *prsn)
{
    ...
    zt_action_frame_del_ba_request(pnic_info);
    ...
    if (pnic_info->buddy_nic)
    {
        ...
        if (get_sys_work_mode(pnic_info->buddy_nic) == ZT_MASTER_MODE)
        {
                              ...
                              zt_s32 *prsn)
{
...

    {
        ...
        zt_memcpy(&ssid.data, pwdn_info->ssid,
                  ZT_MIN(sizeof(zt_80211_mgmt_ssid_t), ZT_80211_MAX_SSID_LEN));
        rst = zt_scan_start(pnic_info, SCAN_TYPE_ACTIVE,
...
zt_inline static zt_bool mlme_can_run(nic_info_st *pnic_info)
{
...

    {
        ...
        if (work_mode != ZT_INFRA_MODE && work_mode != ZT_ADHOC_MODE)
        {
            return zt_false;
        }
    }
...
static zt_s32 mlme_set_state(nic_info_st *pnic_info, mlme_state_e state)
{
    ...
    zt_lock_lock(&pmlme_info->state_lock);
    pmlme_info->state = state;
    zt_lock_unlock(&pmlme_info->state_lock);
...
zt_s32 mlme_set_connect(nic_info_st *pnic_info, zt_bool bconnect)
{
    ...
    zt_lock_lock(&pmlme_info->connect_lock);
    pmlme_info->connect = bconnect;
    zt_lock_unlock(&pmlme_info->connect_lock);
...
zt_s32 zt_mlme_abort(nic_info_st *pnic_info)
{
    ...
    if (get_sys_work_mode(pnic_info) == ZT_INFRA_MODE)
    {
...
zt_s32 zt_mlme_init(nic_info_st *pnic_info)
{
    ...
    zt_lock_init(&pmlme_info->state_lock, ZT_LOCK_TYPE_IRQ);
    zt_lock_init(&pmlme_info->connect_lock, ZT_LOCK_TYPE_SPIN);
    mlme_set_connect(pnic_info, zt_false);
    pmlme_info->freeze_pend = zt_true;
...
zt_s32 zt_mlme_term(nic_info_st *pnic_info)
{
    ...
    MLME_DBG("del lock");
    zt_lock_term(&pmlme_info->state_lock);
    zt_lock_term(&pmlme_info->connect_lock);

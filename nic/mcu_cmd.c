...
static zt_s32 mcu_get_buddy_mlmestate(nic_info_st *pnic_info)
{
    ...
    else if (work_mode == ZT_INFRA_MODE)
    {
...
static zt_s32 mcu_media_connect_set(nic_info_st *nic_info,
                                    wdn_net_info_st *wdn_net_info)
{
    zt_s32 ret = 0;

    ret = mcu_media_status_set(nic_info, zt_true, zt_false, zt_false,
                               CONTROL_ARS_AP, wdn_net_info->wdn_id, zt_false, 0);
    return ret;
}
                                  ...
                                  wdn_net_info_st *wdn_net_info)
{
    ...
    LOG_D("[%s] MacID:%d  RaID:%d  BW:%d  SGI:%d ra_mask: 0x%x", __func__,  buf[0],
          buf[1], buf[2], buf[3], buf[4]);

    ret = mcu_cmd_communicate(nic_info, UMSG_OPS_MSG_SET_RATE_BITMAP, buf, 5,
                              &rate_bitmap, 1);
...
zt_s32 zt_mcu_cmd_get_status(nic_info_st *nic_info, zt_u32 cmd)
{
    ...
    zt_u32 data = 0;
    zt_u32 tryCnt = 0;
    zt_timer_t timeout, tm_intv;
    zt_u32 t_delta = 0;

    // set mailbox zt_s32 finish
    ret = zt_io_write32(nic_info, ZT_MAILBOX_INT_FINISH, 0x12345678);
    if (ZT_RETURN_FAIL == ret)
    {
    ...
    ret = zt_io_write8(nic_info, ZT_MAILBOX_REG_INT, 1);
    if (ZT_RETURN_FAIL == ret)
    {
    ...
    zt_timer_set(&timeout, 1000);
    do
    {
        zt_s32 err = 0;
        if (nic_info->is_surprise_removed || nic_info->is_driver_stopped)
        {
            return ZT_RETURN_REMOVED_FAIL;
        }

        zt_timer_set(&tm_intv, t_delta += (tryCnt++ < 3));

        data = zt_io_read32(nic_info, ZT_MAILBOX_INT_FINISH, &err);
        if (err)
        {
            if (ZT_RETURN_REMOVED_FAIL == err)
            {
        ...
        while (!zt_timer_expired(&tm_intv));
...
{
    zt_s32 ret = 0;
    zt_u32 inbuff[2] = {0xffffffff, 0xffffffff};
...
zt_s32 zt_mcu_set_macid_wakeup(nic_info_st *nic_info, zt_u32 wdn_id)
{
    ...
    arg[0] = WLAN_HAL_VALUE_MACID_WAKEUP;
    arg[1] = wdn_id;
...
zt_s32 zt_mcu_set_correct_tsf(nic_info_st *nic_info, zt_u64 tsf)
{
    ...
    arg[0] = WLAN_HAL_VALUE_CORRECT_TSF;
    arg[1] = (zt_u32)tsf;
    arg[2] = tsf >> 32;
    ...
    LOG_D("[zt_mcu_set_correct_tsf]TSF:%x   %x", arg[1], arg[0]);

    if (NIC_USB == nic_info->nic_type)
    {
        ret = mcu_cmd_communicate(nic_info, UMSG_OPS_HW_SET_CORRECT_TSF, arg, 6, NULL,
                                  0);
    }
    else
    {
...
zt_s32 zt_mcu_set_min_ampdu_space(nic_info_st *pnic_info, zt_u8 min_space)
{
    ...
    arg[0] = WLAN_HAL_VALUE_AMPDU_MIN_SPACE;
    arg[1] = min_space;
                            ...
                            zt_u32 agg_timeout, zt_u32 agg_dma_enable)
{
    ...
    ret = mcu_cmd_communicate(nic_info, UMSG_OPS_HAL_SET_USB_AGG_CUSTOMER, mbox, 3,
                              NULL, 0);
    if (ZT_RETURN_FAIL == ret)
    {
        LOG_E("[%s] failed", __func__);
                                ...
                                wdn_net_info_st *wdn_net_info)
{
    ...
    ret = zt_mcu_msg_sta_info_set(nic_info, &msg_sta);
...
zt_s32 zt_mcu_hw_init(nic_info_st *nic_info, hw_param_st *param)
{
    ...
    ret = mcu_cmd_communicate(nic_info, UMSG_OPS_HAL_FW_INIT, (zt_u32 *)param, 9,
                              NULL, 0);
...
{
    zt_s32 ret = 0;
    zt_u32 len = (info_len / 4) + ((info_len % 4) ? 1 : 0);
    LOG_I("[%s,%d] info_len:%d, len:%d", __func__, __LINE__, info_len, len);
    ret = mcu_cmd_communicate(nic_info, UMSG_OPS_ARS_SET, (zt_u32 *)info, len,
                              NULL, 0);
    if (ZT_RETURN_FAIL == ret)
    {
        LOG_E("[%s] failed", __func__);
                               ...
                               zt_u32 info_len)
{
    ...
    zt_u32 len = (info_len / 4) + ((info_len % 4) ? 1 : 0);
    LOG_I("[%s,%d] info_len:%d, len:%d", __func__, __LINE__, info_len, len);
    ret = mcu_cmd_communicate(nic_info, UMSG_OPS_ARS_DBG_INFO, NULL, 0,
                              (zt_u32 *)info, len);
    if (ZT_RETURN_FAIL == ret)
    {
        LOG_E("[%s] failed", __func__);
                                  ...
                                  wdn_net_info_st *pwdn_info)
{
    ...
    ret = mcu_media_status_set(pnic_info, zt_true, zt_false, zt_false,
                               CONTROL_ARS_STA, pwdn_info->wdn_id, 0, 0);
    return ret;
}
                                     ...
                                     wdn_net_info_st *pwdn_info)
{
    ...
    ret = mcu_media_status_set(pnic_info, zt_false, zt_false, zt_false,
                               CONTROL_ARS_STA, pwdn_info->wdn_id, 0, 0);
    return ret;
}
                                     ...
                                     wdn_net_info_st *pwdn_info)
{
    ...
    ret = mcu_media_status_set(pnic_info, zt_true, zt_false, zt_false,
                               CONTROL_ARS_ADHOC, pwdn_info->wdn_id, 0, 0);
    return ret;
}

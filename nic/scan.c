...
zt_inline static
zt_s32 null_data_frame_send(nic_info_st *pnic_info, zt_bool ps_en)
{
    ...
    ret = zt_nic_mgmt_frame_xmit_with_ack(pnic_info, pwdn_info,
...
static zt_s32 ps_mode(nic_info_st *pnic_info, zt_bool ps_en)
{
    ...
    if (is_connected)
    {
        while (null_data_frame_send(pnic_info, ps_en) && try_cnt[0]--);
    }
    ...
    if (pnic_info->buddy_nic)
    {
        ...
        if (get_sys_work_mode(pnic_buddy) == ZT_INFRA_MODE)
        {
            zt_mlme_get_connect(pnic_buddy, &is_connected);
            if (is_connected)
            {
                while (null_data_frame_send(pnic_buddy, ps_en) && try_cnt[1]--);
            }
        }
    }
...
zt_s32 scan_setting(nic_info_st *pnic_info)
{
    ...
    if (zt_hw_info_get_channnel_bw(pnic_info,
...
zt_inline static zt_s32 scan_setting_recover(nic_info_st *pnic_info)
{
    ...
    if (zt_hw_info_set_channnel_bw(pnic_info,
                                   pscan_info->chnl_bak.number,
                                   pscan_info->chnl_bak.width,
...
zt_pt_ret_t zt_scan_thrd(zt_pt_t *pt, nic_info_st *pnic_info, zt_s32 *prsn)
{
            ...
            pscan_info->ch_idx++)
    {
        ...
        rst = zt_hw_info_set_channnel_bw(pnic_info,
        ...
        {
            if (pnic_info->buddy_nic &&
                    get_sys_work_mode(pnic_info->buddy_nic) == ZT_MASTER_MODE &&

...
/* macros for extraction/creation of zt_u8/zt_u16 values  */
#define RotR1(v16)      ((((v16) >> 1) & 0x7FFF) ^ (((v16) & 1) << 15))
#define Lo8(v16)        ((zt_u8) ((v16)         & 0x00FF))
#define Hi8(v16)        ((zt_u8) (((v16) >> 8)  & 0x00FF))
#define Lo16(v32)       ((zt_u16)((v32)         & 0xFFFF))
#define Hi16(v32)       ((zt_u16)(((v32) >> 16) & 0xFFFF))
#define Mk16(hi, lo)    ((lo) ^ (((zt_u16)(hi)) << 8))

/* select the Nth 16-bit word of the temporal key zt_u8 array TK[]   */
...
/* 2-zt_u8 by 2-zt_u8 subset of the full AES S-box table */
static const zt_u16 Sbox1[2][256]
=    /* Sbox for hash (can be in ROM) */
{
    ...
    {  /* second half of table is zt_u8-reversed version of first! */
...
**********************************************************************
*/
static void phase1(zt_u16 *p1k, const zt_u8 *tk, const zt_u8 *ta, zt_u32 iv32)
{
    zt_s32 i;
            ...
            i++)   /* Each add operation here is mod 2**16 */
    {
        ...
        p1k[4] += (zt_u16)i;    /* avoid "slide attacks" */
    }
}
...
**********************************************************************
*/
static void phase2(zt_u8 *rc4key, const zt_u8 *tk, const zt_u16 *p1k,
                   zt_u16 iv16)
{
    zt_s32  i;
    zt_u16 PPK[6];          /* temporary key for mixing    */
...
zt_s32 tkip_encrypt(struct xmit_frame *pxmitframe, zt_u8 *pdata, zt_u32 len)
{
    ...
    pnl = (zt_u16)(dot11txpn.val); /* TSC0~1 */
    pnh = (zt_u32)(dot11txpn.val >> 16); /* TSC2~5 */
    phase1((zt_u16 *)&ttakey[0], prwskey, pxmitframe->pwlanhdr->addr2, pnh);
    phase2(&rc4key[0], prwskey, (zt_u16 *)&ttakey[0], pnl);

    /* calculate ICV */
    payload = &pframe[payload_ofs];
    //*((__le32 *)crc) = get_crc32(payload, payload_len);
    *((zt_u32 *)crc) = get_crc32(payload, payload_len);
    /* encrypt MSDU */
    arc4_init(&acr4_context, rc4key, 16);
    arc4_encrypt(&acr4_context, payload, payload, payload_len);
    /* append ICV field */
    arc4_encrypt(&acr4_context, &pframe[icv_ofs], crc,
                 pxmitframe->icv_len); /* icv */

    return res;
}


zt_s32 tkip_decrypt(prx_pkt_t ppkt)
{
    ...
    {
        pwdn_info = zt_wdn_find_info(pnic_info, pkt_info->tx_addr);
        if (pwdn_info == NULL)
        {
    ...
    iv = &pframe[iv_ofs];
    GET_TKIP_PN(iv, dot11txpn); /* get TCS0~7 */
    pnl = (zt_u16)(dot11txpn.val); /* TCS0~1 */
    pnh = (zt_u32)(dot11txpn.val >> 16); /* TCS2~5 */
    phase1((zt_u16 *)&ttakey[0], prwskey, pkt_info->tx_addr, pnh);
    phase2(&rc4key[0], prwskey, (zt_u16 *)&ttakey[0], pnl);

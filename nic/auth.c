...
void wdn_info_update(nic_info_st *pnic_info, wdn_net_info_st *pwdn_info)
{
    ...
    zt_memcpy(pwdn_info->bssid, nic_to_local_addr(pnic_info),
                           ...
                           zt_80211_mgmt_t *pmgmt, zt_u16 mgmt_len)
{
    ...
    if (pwdn_info == NULL)
    {
        AUTH_DBG("create new wdn: "ZT_MAC_FMT, ZT_MAC_ARG(pmgmt->sa));
    ...
    else if (pwdn_info->mode != ZT_MASTER_MODE)
    {
                             ...
                             zt_80211_mgmt_t *pmgmt, zt_u16 mgmt_len)
{
    ...
    if (mgmt_len > ZT_80211_MGMT_DEAUTH_SIZE_MAX)
    {
    ...
    if (zt_ap_msg_load(pnic_info, &pwdn_info->ap_msg,
                       ZT_AP_MSG_TAG_DEAUTH_FRAME, pmgmt, mgmt_len))
    {
        return -5;
    }

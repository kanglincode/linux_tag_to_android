...
inline zt_s32 create_skb(prx_pkt_t prx_pkt, zt_u8 *pbuf, zt_u32 pkt_len)
{
...
inline zt_u8 *pkt_pull(prx_pkt_t prx_pkt, zt_s32 size)
{
...
inline zt_u8 *pkt_trim(prx_pkt_t prx_pkt, zt_s32 size)
{
...
inline void upload_skb(nic_info_st *nic_info, void *pskbv)
{
    ...
    else if ((zt_mlme_check_mode(nic_info, ZT_MASTER_MODE) == zt_true))
    {
        ...
        {
            // forward it
            if (0 != zt_tx_msdu(nic_info, pskb->data, pskb->len, pskb))
            {
...
void mpdu_process(struct net_device *ndev, zt_u32 tot_len, zt_u32 remain_len,
                  zt_u8 *buf, zt_u16 buf_size)
{
    zt_s32 ret;
    rx_pkt_t pkt;
    ...
    if (pkt.pkt_info.phy_status)
    {
        ...
        zt_u8 cck_sig_qual_or_ofdm_pwdb = pkt.pdata[RXD_SIZE + 4];
        zt_u8 cck_agc_rpt_or_ofdm_cfo  = pkt.pdata[RXD_SIZE + 5];
        zt_s8 rx_evm =  pkt.pdata[RXD_SIZE + 13];
...
zt_s32 rx_work(struct net_device *ndev, struct sk_buff *skb)
{
    ...
    do
    {
        pkt_len = zt_rx_get_pkt_len_and_check_valid(pbuf, remain_len, &valid, &notice);
        if (valid != zt_true)
        {

...
zt_inline static void lock_mutex_lock(zt_lock_mutex *mtx)
{
    mutex_lock(mtx);
}

zt_inline static zt_s32 lock_mutex_try_lock(zt_lock_mutex *mtx)
{
    return mutex_trylock(mtx) ? 0 : -1;
}

zt_inline static void lock_mutex_unlock(zt_lock_mutex *mtx)
{
    mutex_unlock(mtx);
}

zt_inline static void lock_mutex_init(zt_lock_mutex *mtx)
{
    mutex_init(mtx);
}

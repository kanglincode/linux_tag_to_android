        ...
        struct cfg80211_crypt *param, zt_u32 param_len)
{
    ...
    if ((param->alg == IW_AUTH_CIPHER_NONE) && !pwdn_info)
    {
        ...
        CFG80211_DBG("clear default encryption keys, keyid=%d", param->idx);
        goto exit;
    }
    ...
    if (zt_80211_is_bcast_addr(param->sta_addr))
    {
                ...
                (param->alg == WLAN_CIPHER_SUITE_WEP104))
        {
            ...
            if (wep_key_idx == 0)
            {
                ...
                CFG80211_ARRAY((void *)param->key, wep_key_len);
                psec_info->dot11PrivacyKeyIndex = wep_key_idx;
        ...
        if (param->alg == WLAN_CIPHER_SUITE_TKIP)
        {
            ...
            CFG80211_DBG("key_idx=%d", param->idx);
            CFG80211_ARRAY(param->key, param->key_len);
                          ...
                          struct key_params *params)
{
    ...
    param = (struct cfg80211_crypt *)zt_vmalloc(param_len);
    if (param == NULL)
    {
        res = -EPERM;
        goto exit;
...
static zt_s32 _set_default_key_cb(struct wiphy *wiphy,
                                  struct net_device *ndev, zt_u8 key_index
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 38)) || defined(COMPAT_KERNEL_RELEASE)
...
void zt_cfg80211_scan_done_event_up(nic_info_st *pnic_info, zt_bool babort)
{
    ...
    info.aborted = babort;
#endif
    zt_lock_lock(&pwdev_info->scan_req_lock);
    ...
    zt_lock_unlock(&pwdev_info->scan_req_lock);
}
                                ...
                                zt_wlan_mgmt_scan_que_node_t *pscan_que_node)
{
    ...
    pframe->seq_ctrl = 0;
    /* element */
    zt_memcpy(&pframe->beacon, pscan_que_node->ies, pscan_que_node->ie_len);
    /* timestamp */
    ...
    signal_dbm = translate_percentage_to_dbm(pscan_que_node->signal_strength) *
                 100; /* mBm (100*dBm) */
...
void zt_cfg80211_scan_complete(nic_info_st *pnic_info)
{
    zt_wlan_mgmt_scan_que_node_t *pscan_que_node = NULL;
    zt_wlan_mgmt_scan_que_for_rst_e scan_que_for_rst;
                            ...
                            , struct cfg80211_scan_request *req)
{
    ...
    if (pwdev_info->block_scan == zt_true)
    {
    ...
    if (req->ssids->ssid != NULL && 0 == zt_memcmp(req->ssids->ssid, "DIRECT-", 7)
...
exit:
    ...
    {
        zt_cfg80211_scan_complete(pnic_info);
                            ...
                            struct cfg80211_ibss_params *params)
{
    ...
    if (get_sys_work_mode(pnic_info) != ZT_ADHOC_MODE)
    {
        res = -EPERM;
        goto exit;
    }
...
#endif
{
    ...
    nic_info_st *pnic_info;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 8, 0))
    zt_u16 frame_type = ZT_BIT(upd->global_stypes << 4);
    bool reg = false;
#endif

    zt_widev_priv_t *pwdev;
    ndev_priv_st *pndev_priv;
                         ...
                         )
{
    ...
    if (!target_mac)
    {
        ...
        zt_ap_deauth_all_sta(pnic_info, ZT_80211_REASON_DEAUTH_LEAVING);
    ...
    if (pwdn_info != NULL)
    {
        ...
        if (E_WDN_AP_STATE_DAUTH != pwdn_info->state &&
                E_WDN_AP_STATE_DASSOC != pwdn_info->state)
        {
            CFG80211_DBG("do not del station "ZT_MAC_FMT, ZT_MAC_ARG(target_mac));
        }
        else
        {
                          ...
                          , enum nl80211_channel_type channel_type)
{
    ...
    zt_hw_info_set_channnel_bw(pnic_info, chan_target, chan_width, chan_offset);
...
zt_s32 zt_cfg80211_alloc(nic_info_st *pnic_info)
{
    ...
    zt_lock_init(&pwidev_priv->scan_req_lock, ZT_LOCK_TYPE_BH);

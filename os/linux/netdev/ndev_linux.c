...
static zt_s32 ndev_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* actually xmit */
            if (0 != zt_tx_msdu(ndev_priv->nic, skb->data, skb->len, skb))
            {
...
extern int zt_android_priv_cmd_ioctl(struct net_device *net, struct ifreq *ifr,
                                     int cmd);
...
zt_s32 ndev_register(nic_info_st *pnic_info)
{
    ...
    {
        zt_sprintf(dev_name, if2name[0] ? if2name : "p2p0");
        ...
        if (pnic_info->nic_type == NIC_USB)
        {
            if (pnic_info->virNic)
            {
                zt_sprintf(dev_name, if2name[0] ? if2name : "vir%d_u%d", pnic_info->hif_node_id,
                           pnic_info->ndev_id);
            }
            else
            {
                zt_sprintf(dev_name, ifname[0] ? ifname : "wlan%d_u%d", pnic_info->hif_node_id,
                           pnic_info->ndev_id);
            }
        }
        else
        {
            if (pnic_info->virNic)
            {
                zt_sprintf(dev_name, if2name[0] ? if2name : "vir%d_s%d", pnic_info->hif_node_id,
                           pnic_info->ndev_id);
            }
            else
            {
                zt_sprintf(dev_name, ifname[0] ? ifname : "wlan%d_s%d", pnic_info->hif_node_id,
                           pnic_info->ndev_id);
            }
        }

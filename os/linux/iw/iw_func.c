                          ...
                          union iwreq_data *wrqu, char *extra)
{
    ...
    zt_hw_info_set_channnel_bw(pnic_info, channel, CHANNEL_WIDTH_20,
                          ...
                          union iwreq_data *wrqu, char *extra)
{
    ...
    if (((is_connected == zt_false)  &&
            (zt_local_cfg_get_work_mode(pnic_info) != ZT_ADHOC_MODE)) ||
            (zt_local_cfg_get_work_mode(pnic_info) == ZT_MONITOR_MODE))
    {
                    ...
                    union iwreq_data *wrqu, char *extra)
{
    ...
    if (!zt_80211_is_valid_bssid(awrq->sa_data))
    {
                    ...
                    union iwreq_data *wrqu, char *extra)
{
    ...
    if (is_connected)
    {
        ...
        else if (zt_local_cfg_get_work_mode(pnic_info) == ZT_INFRA_MODE)
        {
            /*check bssid in wdn */
...
zt_s32 zt_iw_set_wpa_ie(nic_info_st *pnic_info, zt_u8 *pie, size_t ielen)
{
    ...
    if (ielen)
    {
        ...
        IW_FUNC_DBG("wpa_ie(length=%d)=", (zt_u16)ielen);

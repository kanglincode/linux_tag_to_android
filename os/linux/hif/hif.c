...
zt_inline zt_s32 hif_nic_tx_wake(nic_info_st *nic_info)
{
...
static zt_s32 __init hif_init(void)
{
    ...
    file = zt_os_api_file_open(zt_cfg);
    if (file == NULL)
    {
    ...
    for (i = 0; i < fw_file_head.firmware_num; i++)
    {
        ...
        if (fw_head.type == 0)
        {
            ...
            gl_hif->fw0 = zt_kzalloc(fw_head.length);
            if (NULL == gl_hif->fw0)
            {
            ...
            zt_os_api_file_read(file, fw_head.offset, (zt_u8 *)gl_hif->fw0,
                                fw_head.length);
        }
        else
        {
            ...
            gl_hif->fw1 = zt_kzalloc(fw_head.length);
            if (NULL == gl_hif->fw1)
            {
            ...
            zt_os_api_file_read(file, fw_head.offset + 32, (zt_u8 *)gl_hif->fw1,
                                fw_head.length);
        }
        pos += sizeof(fw_head);
    }
    ...
    zt_lock_init(hm_get_lock(), ZT_LOCK_TYPE_MUTEX);
...
zt_inline static void hif_hw_access_init(hif_node_st *hif_info)
{
    ...
    zt_lock_init(&hif_info->mlme_hw_access_lock, ZT_LOCK_TYPE_MUTEX);

    /* mcu hardware access hw lock */
    zt_lock_init(&hif_info->mcu_hw_access_lock, ZT_LOCK_TYPE_MUTEX);
}
...
{
    hif_node->reg_buffer =  zt_kzalloc(512);
...
void zt_hif_bulk_reg_post(hif_node_st *hif_node, zt_u8 *buff, zt_u16 len)
{
    if (len <= 512)
    {
        zt_memcpy(hif_node->reg_buffer, buff, len);

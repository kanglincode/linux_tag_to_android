                                  ...
                                  void *param), void *tx_info, void *param)
{
    ...
    zt_lock_lock(&trxq->queu_txop_lock);
    ...
    if (zt_data_queue_full(tx_queue, ZT_TX_MAX_DATA_QUEUE_NODE_NUM))
    {
        ...
        zt_lock_unlock(&trxq->queu_txop_lock);
        return ZT_RETURN_FAIL;
    }

    /*get node from free_tx_queue*/
    zt_timer_set(&timer, 100);
    do
    {
        tmp_cnt++;
        if (zt_timer_expired(&timer))
        {
            zt_timer_reset(&timer);
...
zt_s32 hif_frame_dispath(hif_node_st *hif_info, struct sk_buff *pskb)
{
    zt_s32 num = 0;
    ...
    //  LOG_D("[hif_frame_dispath]:mac1:"ZT_MAC_FMT,ZT_MAC_ARG(get_ra(pdata)));
    //  LOG_D("[hif_frame_dispath]:ta:"ZT_MAC_FMT,ZT_MAC_ARG(get_ta(pdata)));
    while (num < hif_info->nic_number)
    {
        ...
        if (zt_memcmp(ndev->dev_addr, get_ra(pdata), ETH_ALEN) == 0)
        {
            //LOG_D("nic[%d]: "ZT_MAC_FMT,num,ZT_MAC_ARG(nic_to_local_addr(hif_info->nic_info[num]) ));
            rx_work(hif_info->nic_info[num]->ndev, pskb);
            return 0;

        }
        else if (zt_memcmp(bc_addr, get_ra(pdata), ETH_ALEN) == 0)
        {
            rx_work(hif_info->nic_info[num]->ndev, pskb);
        }
        num++;
...
zt_s32 zt_data_queue_mngt_init(void *hif_node)
{
    ...
    zt_lock_init(&data_queue_mngt->queu_txop_lock, ZT_LOCK_TYPE_NONE);
    ...
    zt_tasklet_init(&data_queue_mngt->recv_task,
                    (void (*)(zt_ptr))hif_tasklet_rx_handle, (zt_ptr)hif_node);
    ...
    zt_tasklet_init(&data_queue_mngt->send_task,
                    (void (*)(zt_ptr))hif_tasklet_tx_handle, (zt_ptr)hif_node);
...
zt_s32 zt_data_queue_mngt_term(void *hif_node)
{
    ...
    /*rx queue freee */
    // zt_lock_lock(&trxq->free_rx_queue.lock); // no need lock, lock will cause usb_kill_urb run bug, by renhaibo
    ...
    //zt_lock_unlock(&trxq->free_rx_queue.lock);
    ...
    //LOG_I("[%s,%d]",__func__,__LINE__);
    /*tx queue free*/
    //zt_lock_lock(&trxq->free_tx_queue.lock);   // no need lock, lock will cause usb_kill_urb run bug, by renhaibo
    ...
    //zt_lock_unlock(&trxq->free_tx_queue.lock);
    zt_que_deinit(&trxq->free_tx_queue);
    zt_que_deinit(&trxq->tx_queue);
    zt_lock_term(&trxq->queu_txop_lock);

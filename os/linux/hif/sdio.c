                                     ...
                                     zt_u8 *sdata, zt_u32 slen)
{
    ...
    ret = zt_sdio_tx_flow_agg_num_check(hif_node, pxmitbuf->agg_num, data_type);
    if (ret == ZT_RETURN_FAIL)
    {
        zt_sdio_tx_wait_freeAGG(hif_node, pxmitbuf->agg_num);
        data_queue_node->state = TX_STATE_FLOW_CTL_SECOND;
    }
    ...
    else
    {
        ...
        zt_sdio_tx_flow_agg_num_ctl(hif_node, pxmitbuf->agg_num);
    }
        ...
        else
        {
            if (zt_rx_cmd_check(pskb->data, pskb->len) == 0)
            {
                switch (zt_rx_data_type(pskb->data))
                {
                    ...
                    case ZT_PKT_TYPE_REG:
                        zt_hif_bulk_reg_post(hif_node, pskb->data, pskb->len);
                        ret = ZT_PKT_TYPE_REG;
                        break;
